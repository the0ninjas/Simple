...
# AU Detection Parameters
AU_INDEX_TO_NUM: List[int] = [1, 2, 4, 6, 9, 12, 25, 26]  # AU numbers corresponding to model output indices
AU_THRESHOLD = 0.2  # Minimum AU intensity to display (0 = show all, 0.5 = only strong activations)

# Heatmap Visualization Parameters
BASE_SIGMA = 25.0  # Maximum Gaussian blur radius in pixels (larger = broader heat areas)
MIN_SIGMA = 15.0   # Minimum Gaussian blur radius in pixels (smaller = more focused heat)
AMPLITUDE_MULTIPLIER = 3  # Multiplier for heat intensity (higher = more visible heat)
HEATMAP_ALPHA = 0.75  # Heatmap overlay transparency (0.0 = invisible, 1.0 = opaque)

# UI Parameters
BAR_START_XY = (10, 70)  # Position of AU intensity bars (x, y)
BAR_SIZE = (180, 11)     # Size of each AU bar (width, height)
BAR_GAP = 10             # Vertical spacing between bars
FONT_SCALE = 0.3         # Text size for labels
...
def add_gaussian(heatmap: np.ndarray, center: Tuple[float, float], amplitude: float, sigma: float, 
                 face_mask: Optional[np.ndarray] = None) -> None:
    """Add a 2D Gaussian to heatmap at given center, optionally masked to face region.

    The Gaussian is applied in a local window of radius ~3*sigma for efficiency.
    If face_mask is provided, only applies heat within the masked region.
    """
    h, w = heatmap.shape[:2]
    ...
    if face_mask is not None:
        mask_region = face_mask[y0:y1, x0:x1]
        g = g * mask_region.astype(np.float32)
...
def normalize_to_uint8(heatmap: np.ndarray) -> np.ndarray:
    ...
    # Use a lower threshold for better visibility of low-intensity heat
    minv = float(hm.min())
    if maxv > minv:
        norm = ((hm - minv) / (maxv - minv)) * 255.0
    else:
        norm = hm * 255.0
...
def create_face_mask(frame_shape: Tuple[int, int], face_bbox: np.ndarray, 
                     expansion_factor: float = 1.2) -> np.ndarray:
    """Create a binary mask for the face region with optional expansion."""
    h, w = frame_shape[:2]
...
def compute_landmark_weights_full_face(landmarks: np.ndarray, au_intensity: np.ndarray, 
                                      au_index_to_num: List[int], au_landmark_map: Dict) -> np.ndarray:
    """Compute weights for all landmarks based on AU intensities for full-face heatmap."""
    ...
    # For each landmark, find which AUs affect it and sum their intensities
    for landmark_idx in range(num_points):
        total_intensity = 0.0
        
        # Check each AU to see if this landmark is affected
        for au_idx, au_num in enumerate(au_index_to_num):
            if au_idx >= au_intensity.size:
                break
            
            # Only include AU intensity if it's above the threshold
            au_value = float(au_intensity[au_idx])
            if au_value < AU_THRESHOLD:
                continue
                
            au_key = f"AU{au_num}"
            region_dict = au_landmark_map.get(au_key, {})
            
            # Check if this landmark is in any region (left, right, centre) for this AU
            for side in ("left", "right", "centre"):
                if landmark_idx in region_dict.get(side, []):
                    total_intensity += au_value
                    break  # Don't double-count if landmark appears in multiple sides
        
        weights[landmark_idx] = total_intensity
...
def draw_au_bars(
    ...
    start_xy: Tuple[int, int] = BAR_START_XY,
    bar_size: Tuple[int, int] = BAR_SIZE,
    gap: int = BAR_GAP,
) -> None:
    ...
    for idx, au_num in enumerate(au_index_to_num):
        ...
        cv2.putText(image, label, (label_x, y + bar_h - 4), font, FONT_SCALE, (230, 230, 230), 1, cv2.LINE_AA)
...
def main() -> None:
    ...
    # Parameters are now defined at the top of the script for easy configuration
    ...
    try:
        while True:
            ...
            if dets is not None and len(dets) > 0 and cropped_face is not None:
                # Create face mask for heatmap isolation
                face_mask = create_face_mask(frame.shape, dets[0])
                ...
                if lm_list:
                    ...
                    # Full-face heatmap: compute weights for all landmarks based on AU intensities
                    landmark_weights = compute_landmark_weights_full_face(lm, au_vals, AU_INDEX_TO_NUM, AU_LANDMARK_MAP)
                    
                    # Create heatmap using all landmarks with their computed weights, masked to face region
                    for i, (x, y) in enumerate(lm.astype(int)):
                        if i < len(landmark_weights):
                            weight = landmark_weights[i]
                            if weight > 0:  # Only add heat for landmarks with non-zero weight
                                # Amplitude and sigma scaled by weight - using configurable parameters
                                amplitude = max(0.0, min(1.0, weight)) * AMPLITUDE_MULTIPLIER
                                sigma = MIN_SIGMA + (BASE_SIGMA - MIN_SIGMA) * amplitude
                                add_gaussian(cumulative, (float(x), float(y)), amplitude=amplitude, sigma=sigma, face_mask=face_mask)
            ...
            # Use configurable alpha for heatmap visibility
            overlay = cv2.addWeighted(vis, 1.0, heat_color, HEATMAP_ALPHA, 0)
            ...
            cv2.putText(overlay, f"Infer: {last_face_ms:.1f} ms", (10, 22), cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, (0, 255, 0), 1, cv2.LINE_AA)
            cv2.putText(overlay, f"AU thr: {AU_THRESHOLD:.2f}    Toggle points: p", (10, 44), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (200, 200, 0), 1, cv2.LINE_AA)

            # Draw AU intensity bars using configurable parameters
            current_au_vals = au_vals
            if current_au_vals is not None:
                draw_au_bars(overlay, current_au_vals, AU_INDEX_TO_NUM)
